...
template<typename T>
t_scoped t_type_of<t_array_of<T>>::f_construct(t_object* a_class, t_stacked* a_stack, size_t a_n)
{
	return t_overload<
...
template<typename T>
size_t t_type_of<t_array_of<T>>::f_get_at(t_object* a_this, t_stacked* a_stack)
{
	t_destruct<> a0(a_stack[2]);
	f_check<size_t>(a0.v_p, L"index");
	a_stack[0].f_construct(f_as<const t_array_of<T>&>(a_this).f_get_at(f_as<size_t>(a0.v_p)));
	return -1;
}

template<typename T>
size_t t_type_of<t_array_of<T>>::f_set_at(t_object* a_this, t_stacked* a_stack)
{
	t_destruct<> a0(a_stack[2]);
	t_destruct<> a1(a_stack[3]);
	f_check<size_t>(a0.v_p, L"index");
	f_check<T>(a1.v_p, L"value");
	a_stack[0].f_construct(f_as<t_array_of<T>&>(a_this).f_set_at(f_as<size_t>(a0.v_p), f_as<T>(a1.v_p)));
	return -1;
}

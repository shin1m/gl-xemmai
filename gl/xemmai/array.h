...
template<typename T>
t_scoped t_type_of<t_array_of<T>>::f_construct(t_object* a_class, t_scoped* a_stack, size_t a_n)
{
	return t_overload<
...
template<typename T>
size_t t_type_of<t_array_of<T>>::f_get_at(t_object* a_this, t_scoped* a_stack)
{
...
template<typename T>
size_t t_type_of<t_array_of<T>>::f_set_at(t_object* a_this, t_scoped* a_stack)
{
	t_scoped a0 = std::move(a_stack[2]);

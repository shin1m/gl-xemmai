...
t_transfer t_program::f_get_attached_shaders() const
{
	...
	t_transfer p = t_tuple::f_instantiate(n);
	t_tuple& tuple = f_as<t_tuple&>(p);
	for (GLint i = 0; i < n; ++i) tuple[i].f_construct(session->v_shaders.find(shaders[i])->second);
	return p;
}

t_transfer t_program::f_get_uniformfv(const t_uniform_location& a_location) const
{
	t_transfer p = t_bytes::f_instantiate(sizeof(GLfloat) * 16);
	t_bytes& bytes = f_as<t_bytes&>(p);
	glGetUniformfv(f_id(), a_location.f_id(), reinterpret_cast<GLfloat*>(&bytes[0]));
	return p;
}

t_transfer t_program::f_get_uniformiv(const t_uniform_location& a_location) const
{
	t_transfer p = t_bytes::f_instantiate(sizeof(GLint) * 16);
	t_bytes& bytes = f_as<t_bytes&>(p);
...
void t_type_of<t_program>::f_define(t_extension* a_extension)
{
	t_define<t_program, t_object>(a_extension, L"Program")
		...
		(L"get_active_attrib", t_member<t_transfer (t_program::*)(GLuint) const, &t_program::f_get_active_attrib>())
		(L"get_active_uniform", t_member<t_transfer (t_program::*)(GLuint) const, &t_program::f_get_active_uniform>())
		(L"get_attached_shaders", t_member<t_transfer (t_program::*)() const, &t_program::f_get_attached_shaders>())
		...
		(L"get_uniformfv", t_member<t_transfer (t_program::*)(const t_uniform_location&) const, &t_program::f_get_uniformfv>())
		(L"get_uniformiv", t_member<t_transfer (t_program::*)(const t_uniform_location&) const, &t_program::f_get_uniformiv>())
...
t_type* t_type_of<t_program>::f_derive(t_object* a_this)
{
	return 0;
}

void t_type_of<t_program>::f_finalize(t_object* a_this)
{
	t_program* p = static_cast<t_program*>(a_this->f_pointer());
	assert(!p);
}

t_transfer t_type_of<t_program>::f_construct(t_object* a_class, t_slot* a_stack, size_t a_n)
{
	return t_construct_with<t_transfer (*)(t_object*), t_program::f_construct>::t_bind<t_program>::f_do(a_class, a_stack, a_n);
}
